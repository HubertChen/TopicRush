...
<?php
  ...
  if ($validtopicid == TRUE) {
    ...
    if ($memberfollows == TRUE) {
      ...
      echo $topicname;
      echo '</h1>';
      echo 'You are no longer following this topic!<br>';
	  */
	  $errorMessage = '<div class="alert alert-success alert-dismissable" align="center">
     						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
							Successfully unfollowed topic!
							</div>';
      $sql = 'delete from follows where memberid=' . $memberid . ' and topicid=' . $topicid;
      mysqli_query($con,$sql);
	  
	  include 'viewtopic.php';
	  exit();
    } else { // end if $memberfollows == TRUE
