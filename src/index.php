...
<?php
  ...
  //navbar: user is logged in           
	...
	include 'index.html.php';
  	...
  	$sql = 'select count(communityid) from Circle.community';
  	$result = mysqli_query($con,$sql);
  	while($row = mysqli_fetch_array($result)) { $totalcommunity = $row['count(communityid)']; }
  	if ($totalcommunity > 2) {
    	$maxcommunity = 3;
  	} else {
  	...
  	$result = mysqli_query($con,$sql);
  	while($row = mysqli_fetch_array($result)) { $totaltopic = $row['count(topicid)']; }
 	if ($totaltopic > 2) {
    	$maxtopic = 3;
  	} else {
    	$maxtopic = $totaltopic;
  	...
  	$result = mysqli_query($con,$sql);
  	while($row = mysqli_fetch_array($result)) { $totalproduct = $row["count(productid)"]; }
  	if ($totalproduct > 2) {
   		$maxproduct = 3;
  	} else {
